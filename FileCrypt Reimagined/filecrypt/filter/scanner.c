...
Abstract:
    ...
    This is the main module of the filecrypt filter.
...
#define filecrypt_REG_TAG       'Rncs'
#define filecrypt_STRING_TAG    'Sncs'

//
//  Structure that contains all the global data structures
//  used throughout the filecrypt.
//

filecrypt_DATA filecryptData;
...
PFN_IoOpenDriverRegistryKey
filecryptGetIoOpenDriverRegistryKey (
    VOID
    );

NTSTATUS
filecryptOpenServiceParametersKey (
...
NTSTATUS
filecryptInitializeScannedExtensions(
    _In_ PDRIVER_OBJECT DriverObject,
    _In_ PUNICODE_STRING RegistryPath
    );

VOID
filecryptFreeExtensions(
    );

NTSTATUS
filecryptAllocateUnicodeString (
    _Inout_ PUNICODE_STRING String
    );

VOID
filecryptFreeUnicodeString (
    _Inout_ PUNICODE_STRING String
    );

NTSTATUS
filecryptPortConnect (
...
VOID
filecryptPortDisconnect (
    _In_opt_ PVOID ConnectionCookie
    );

NTSTATUS
filecryptpScanFileInUserMode (
...
//
    ...
    #pragma alloc_text(INIT, filecryptGetIoOpenDriverRegistryKey)
    #pragma alloc_text(INIT, filecryptOpenServiceParametersKey)
    #pragma alloc_text(INIT, filecryptInitializeScannedExtensions)
    #pragma alloc_text(PAGE, filecryptInstanceSetup)
    #pragma alloc_text(PAGE, filecryptPreCreate)
    #pragma alloc_text(PAGE, filecryptPortConnect)
    #pragma alloc_text(PAGE, filecryptPortDisconnect)
    #pragma alloc_text(PAGE, filecryptFreeExtensions)
    #pragma alloc_text(PAGE, filecryptAllocateUnicodeString)
    #pragma alloc_text(PAGE, filecryptFreeUnicodeString)
#endif
...
const FLT_CONTEXT_REGISTRATION ContextRegistration[] = {
    ...
    { FLT_STREAMHANDLE_CONTEXT,
      0,
      NULL,
      sizeof(filecrypt_STREAM_HANDLE_CONTEXT),
      'chBS' },
...
const FLT_REGISTRATION FilterRegistration = {
    ...
    filecryptUnload,                      //  FilterUnload
    filecryptInstanceSetup,               //  InstanceSetup
    filecryptQueryTeardown,               //  InstanceQueryTeardown
...
--*/
{
    ...
    status = filecryptInitializeScannedExtensions( DriverObject, RegistryPath );
    ...
    RtlInitUnicodeString( &uniString, filecryptPortName );
    ...
    if (NT_SUCCESS( status )) {
        ...
        status = FltCreateCommunicationPort( filecryptData.Filter,
                                             &filecryptData.ServerPort,
                                             &oa,
                                             NULL,
                                             filecryptPortConnect,
                                             filecryptPortDisconnect,
        ...
        if (NT_SUCCESS( status )) {
            ...
            status = FltStartFiltering( filecryptData.Filter );
            ...
            FltCloseCommunicationPort( filecryptData.ServerPort );
        }
    }

    filecryptFreeExtensions();

    FltUnregisterFilter( filecryptData.Filter );
...
--*/
{
    ...
    pIoOpenDriverRegistryKey = filecryptGetIoOpenDriverRegistryKey();
    ...
    if (pIoOpenDriverRegistryKey != NULL) {
        ...
        if (!NT_SUCCESS( status )) {
            ...
            goto filecryptOpenServiceParametersKeyCleanup;
        }
...
filecryptOpenServiceParametersKeyCleanup:
...
--*/
{
    ...
    status = filecryptOpenServiceParametersKey( DriverObject,
                                              RegistryPath,
                                              &driverRegKey );
    ...
    if (!NT_SUCCESS( status )) {
        ...
        driverRegKey = NULL;
        goto filecryptInitializeScannedExtensionsCleanup;
    }
...
FLT_FILESYSTEM_TYPE
    ...
    while (ScannedExtensionCount < count) {
        ...
        status = filecryptAllocateUnicodeString( ext );
...
filecryptInitializeScannedExtensionsCleanup:
    ...
    if (valueBuffer != NULL) {
        ...
        ExFreePoolWithTag( valueBuffer, filecrypt_REG_TAG );
        valueBuffer = NULL;
    }
...
--*/
{
    ...
    while (ScannedExtensionCount > 0) {
        ...
        if (ScannedExtensions != &ScannedExtensionDefault) {
            ...
            filecryptFreeUnicodeString( ScannedExtensions + ScannedExtensionCount );
        }
    }

    if (ScannedExtensions != &ScannedExtensionDefault && ScannedExtensions != NULL) {

        ExFreePoolWithTag( ScannedExtensions, filecrypt_STRING_TAG );
    }
...
--*/
{
    ...
    FLT_ASSERT( filecryptData.ClientPort == NULL );
    FLT_ASSERT( filecryptData.UserProcess == NULL );
    ...
    filecryptData.UserProcess = PsGetCurrentProcess();
    filecryptData.ClientPort = ClientPort;

    DbgPrint( "!!! filecrypt.sys --- connected, port=0x%p\n", ClientPort );
...
--*/
{
    ...
    DbgPrint( "!!! filecrypt.sys --- disconnected, port=0x%p\n", filecryptData.ClientPort );
    ...
    FltCloseClientPort( filecryptData.Filter, &filecryptData.ClientPort );

    //
    //  Reset the user-process field.
    //

    filecryptData.UserProcess = NULL;
}
...
--*/
{
    ...
    FLT_ASSERT( FltObjects->Filter == filecryptData.Filter );
...
--*/
{
    ...
    if (IoThreadToProcess( Data->Thread ) == filecryptData.UserProcess) {

        DbgPrint( "!!! filecrypt.sys -- allowing create for trusted process \n" );
...
--*/
{
    Pfilecrypt_STREAM_HANDLE_CONTEXT filecryptContext;
    ...
    scanFile = filecryptpCheckExtension( &nameInfo->Extension );
    ...
    (VOID) filecryptpScanFileInUserMode( FltObjects->Instance,
    ...
    if (!safeToOpen) {
        ...
        DbgPrint( "!!! filecrypt.sys -- foul language detected in postcreate !!!\n" );

        DbgPrint( "!!! filecrypt.sys -- undoing create \n" );
        ...
        status = FltAllocateContext( filecryptData.Filter,
                                     FLT_STREAMHANDLE_CONTEXT,
                                     sizeof(filecrypt_STREAM_HANDLE_CONTEXT),
                                     PagedPool,
                                     &filecryptContext );
        ...
        if (NT_SUCCESS(status)) {
            ...
            filecryptContext->RescanRequired = TRUE;
            ...
            FltReleaseContext( filecryptContext );
        }
    }
...
{
    NTSTATUS status;
    Pfilecrypt_STREAM_HANDLE_CONTEXT context;
    BOOLEAN safe;
    ...
    if (NT_SUCCESS( status )) {
        ...
        if (context->RescanRequired) {
            ...
            if (!safe) {
                ...
                DbgPrint( "!!! filecrypt.sys -- foul language detected in precleanup !!!\n" );
            }
        }
...
--*/
{
    ...
    NTSTATUS status;
    Pfilecrypt_NOTIFICATION notification = NULL;
    Pfilecrypt_STREAM_HANDLE_CONTEXT context = NULL;
    ULONG replyLength;
    BOOLEAN safe = TRUE;
    PUCHAR buffer;
    ...
    if (filecryptData.ClientPort == NULL) {
    ...
    try {
        ...
        if (Data->Iopb->Parameters.Write.Length != 0) {
            ...
            FsRtlIsMobileOS
            notification->BytesToScan = min( Data->Iopb->Parameters.Write.Length, filecrypt_READ_BUFFER_SIZE );
            ...
            replyLength = sizeof( filecrypt_REPLY );

            status = FltSendMessage( filecryptData.Filter,
                                     &filecryptData.ClientPort,
                                     notification,
                                     sizeof( filecrypt_NOTIFICATION ),
                                     notification,
            ...
            if (STATUS_SUCCESS == status) {
               ...
               safe = ((Pfilecrypt_REPLY) notification)->SafeToOpen;
               ...
               DbgPrint( "!!! filecrypt.sys --- couldn't send message to user-mode to scan file, status 0x%X\n", status );
           }
        }
        ...
        if (!safe) {
            ...
            //
            //  Block this write if not paging i/o (as a result of course, this filecrypt will not prevent memory mapped writes of contaminated
            ...
            DbgPrint( "!!! filecrypt.sys -- foul language detected in write !!!\n" );

            if (!FlagOn( Data->Iopb->IrpFlags, IRP_PAGING_IO )) {

                DbgPrint( "!!! filecrypt.sys -- blocking the write !!!\n" );
...
--*/
{
    ...
    try {
        ...
        if (fsControlCode == FSCTL_OFFLOAD_WRITE) {
            ...
            //
            //  filecrypt cannot access the data in this offload write request.
            ...
            DbgPrint( "!!! filecrypt.sys -- blocking the offload write !!!\n" );
...
Arguments:
    ...
    Instance - Handle to the filter instance for the filecrypt on this volume.
...

{
    ...
    *SafeToOpen = TRUE;
    ...
    try {
        ...
        length = max( filecrypt_READ_BUFFER_SIZE, volumeProps.SectorSize );
        ...
        if (NT_SUCCESS( status ) && (0 != bytesRead)) {
            ...
            RtlCopyMemory( &notification->Contents,
                           buffer,
                           min( notification->BytesToScan, filecrypt_READ_BUFFER_SIZE ) );
            ...
            if (STATUS_SUCCESS == status) {
                ...
                *SafeToOpen = ((Pfilecrypt_REPLY) notification)->SafeToOpen;
